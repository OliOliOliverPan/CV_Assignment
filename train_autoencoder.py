...
class SegmentationAugment:
    ...
    def __call__(self, image, mask):
        ...
        For autoencoder pre-training, the same image is passed for both image and mask,
        in which case identical transformations are applied (i.e. same fill values).
        """
        ...
        if op == 'flip':
            ...
            if same:
                ...
                image = TF.rotate(image, angle, fill=0)
                mask = TF.rotate(mask, angle, fill=0)  # 255 → unknown region
        elif op == 'affine':
            if same:
...
# ------------------------------
# 1. Define the Enhanced UNet Architecture
# ------------------------------
...
class UpBlock(nn.Module):
    ...
    def forward(self, x, skip):
        x = self.up(x)
        # Handle potential size mismatches by padding
        if x.size() != skip.size():
